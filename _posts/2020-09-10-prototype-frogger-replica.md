...
You might quickly realize that you can move the frog beyond the left, right, and bottom screen boundaries. This, obviously, would need to be corrected. An easy way of doing that would be to use the code recipe I’ve previously created from [Prototype 2 - Bubble Struggle Replica](https://www.oncollisionenter.com/2020/09/07/prototype-bubble-struggle-replica.html). Basically, it would involve clamping how far the player can move on the x- and y-axis.
...
The frog sprite is also very simple. It could have been improved by having several versions of the frog in various animation states, then using the Animator and Animation windows in Unity to build some animation states and transitions that trigger when the frog starts moving.
...
= Should you use a public static int vs a getter/setter?
A static variable retains its value while the game is running. So if you change scenes, the static variable keeps its value. Sometimes this is desired and sometimes it isn’t.
